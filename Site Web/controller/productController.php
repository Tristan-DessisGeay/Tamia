...
class ProductController {
    ...
    public static function use() {
        if(ProductModel::testUserEtProduitCorrects($_SESSION['id_u'], $_GET['id_p'])) {
            ...
            $id_s = $product->get('id_s');
            $id_p = $_GET['id_p'];
            $currentSite = SiteModel::getSiteFromID($product->get('id_s'));
            $propertyid_p = $currentSite->get('propertyid_s');
            $eventlist2_p = $product->get('eventlist_p');
            $event_p = $product->get('event_p');
