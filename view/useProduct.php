    ...
    <div id="resultatCadre">
        <div id="eventList">
            <?php for($i=0;$i<count($eventlist_p);$i++) { ?>
                <div><p><?php echo $eventlist_p[$i] ?></p><span>(?)</span></div>
            <?php } ?>
        </div>
        <img src="assets/image/lianes.png">
        <div id="resultCadre">
            <p id="re">? vente(s) estimée(s)</p>
        </div>
    </div>
        ...
        function curveMAJ() {
            ...
            */
            $.ajax({
                ...
                data: {site:"1",produit:"1"} // Changer dynamiquement l'id du site et celui du produit
            }).done(function(data){
                if(data!=0) {
                    data = data.split(";")
                }else{
